    ...
    if inputs.any():
        ...
        if type(ip     ) == type(np.asarray([])) and \
           ...
           type(ip[0,0]) == type(np.asarray([])):
            ...
            kM   = ceil(sqrt(len(ip[0,0])/calcN(len(ip[0,0]))))
            ...
            if ver == const.VER:
                enc  = Conv2D(nb_filter=M,nb_row=kM,nb_col=kM,input_shape=ip[0].shape,subsample=1,activation='tanh',border_mode='same',init='random_normal')
                ...
                enc  = Conv2D(filters=M,kernel_size=kM,input_shape=ip[0].shape,strides=1,activation='selu',padding='same',kernel_initializer='random_normal')
        ...
        if not (type(dbnact) == type(None) or dbnout <= 0):
            ...
            if type(ip     ) == type(np.asarray([])) and \
               ...
               type(ip[0,0]) == type(np.asarray([])):
                ...
                if ver == const.VER:
                    enc  = Conv2DTranspose(nb_filter=M,nb_row=kM,nb_col=kM,input_shape=ip[0].shape,subsample=1,activation='tanh',border_mode='same',init='random_normal')
                    ...
                    enc  = Conv2DTranspose(filters=M,kernel_size=kM,input_shape=ip[0].shape,strides=1,activation='selu',padding='same',kernel_initializer='random_normal')
        ...
        # save the model to the file defined by sfl
        if not (type(sfl) == type(None)):
            # construct the relaxed image name
            #
            # file name minus the type extension
...
def nn(fl=None):
    ...
    if not (fl == None):
        ...
        if (type(fl) == type([]) or type(fl) == type(np.asarray([]))):
            ...
            #
            # file name minus the type extension
            fln  = const.SFL[:const.SFL.rfind(".") ]
            # file type extension
            flt  = const.SFL[ const.SFL.rfind("."):]
            # current date and time
            dt   = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
            sfl  = fln + "_" + dt + flt
            ...
            preds = np.where(preds<=(const.PRED_SHIFT if hasattr(const,"PRED_SHIFT") else 10),0,preds-(const.PRED_SHIFT if hasattr(const,"PRED_SHIFT") else 10))
            # reconstitute the diff image
            oivals= np.asarray(recon(oivals))
            oivals= oivals.reshape(np.asarray(oivals.shape)[sorted(range(0,len(oivals.shape)),reverse=True)])
            print(oivals.shape)
            # define the relaxed image
            Image.fromarray(oivals.astype(np.uint8)).save("/tmp/oivals.jpg")
            Image.fromarray(preds.astype(np.uint8)).save("/tmp/preds.jpg")
            rivals= oivals.astype(np.uint8) - preds.astype(np.uint8)
