...
def nn(fl=None):
    ...
    if not (fl == None):
        ...
        if (type(fl) == type([]) or type(fl) == type(np.asarray([]))):
            ...
            preds = np.where(ivals-rivals<=(const.PRED_L_SHIFT if hasattr(const,"PRED_L_SHIFT") else 10),0,preds-(const.PRED_L_SHIFT if hasattr(const,"PRED_L_SHIFT") else 10)      )
            preds = np.where(ivals-rivals>=(const.PRED_H_SHIFT if hasattr(const,"PRED_H_SHIFT") else 15),0,      (const.PRED_H_SHIFT if hasattr(const,"PRED_H_SHIFT") else 15)-preds)
